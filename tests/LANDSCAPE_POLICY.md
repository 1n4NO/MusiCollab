...
The iPhone 14 implementation enforces the orientation at the view-controller level and uses an edge-to-edge landscape stack: no extra horizontal content gutters are added beyond each panel’s internal control padding. Companion and Composer guidance remains a web-surface responsibility because Safari and desktop browsers control the outer window orientation. All controls must remain usable at the supported landscape viewport, with no required portrait-only workflow.
